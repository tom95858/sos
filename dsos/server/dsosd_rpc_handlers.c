...
static dsos_handle_t dsosd_ptr_to_handle(dsosd_rpc_t *rpc, void *ptr, int type)
{
	struct ptr_rbn	*rbn;
	...
	dsosd_debug("client %p ptr %p assigned handle 0x%lx type %d\n",
		    client, ptr, handle, type);

	return handle;
}

static void *dsosd_handle_to_ptr(dsosd_rpc_t *rpc, dsos_handle_t handle)
{
	struct ptr_rbn	*rbn;
	dsosd_client_t	*client = rpc->client;
...
static void dsosd_handle_free(dsosd_rpc_t *rpc, dsos_handle_t handle)
{
	struct ptr_rbn	*rbn;
	dsosd_client_t	*client = rpc->client;
...
void rpc_handle_obj_create(dsosd_rpc_t *rpc)
{
	...
	schema = dsosd_handle_to_ptr(rpc, handle);
	if (!schema) {
	...
	if (flags & DSOS_RPC_FLAGS_INLINE) {
		...
		*(uint64_t *)obj_data = sos_schema_id(schema);
		...
		dsosd_debug("rpc %p new inline obj %p obj_data %p sz %d id %08lx%08lx\n", rpc,
			    obj, obj_data, obj_sz, obj->obj_ref.ref.ods, obj->obj_ref.ref.obj);
		dsosd_rpc_complete(rpc, 0);
	} else {
...
void rpc_handle_obj_delete(dsosd_rpc_t *rpc)
{
	...
	cont = (sos_t)dsosd_handle_to_ptr(rpc, handle);
	if (!cont) {
		ret = EBADF;
		goto out;
	}
	...
	ret = 0;
 out:
	dsosd_debug("rpc %p: cont %lx/%p part %p obj %p %08lx%08lx status %d\n", rpc,
		    handle, cont, part, obj, obj_ref.ref.ods, obj_ref.ref.obj, ret);
...
void rpc_handle_cont_open(dsosd_rpc_t *rpc)
{
	...
	dsosd_debug("rpc %d: '%s' perms 0%o, cont %p\n", rpc, path, perms, cont);
...
void rpc_handle_schema_by_name(dsosd_rpc_t *rpc)
{
	int		ret = 0;
	char		*schema_name;
	dsos_handle_t	cont_handle, schema_handle;
	sos_t		cont;
	sos_schema_t	schema;

	cont_handle = dsosd_rpc_unpack_handle(rpc);
	schema_name = dsosd_rpc_unpack_str(rpc);

	cont = (sos_t)dsosd_handle_to_ptr(rpc, cont_handle);
	if (!cont) {
		ret = EBADF;
		goto out;
	}
 ...
 out:
	dsosd_debug("rpc %p: cont %lx/%p schema %lx/%p status %d\n", rpc,
		    cont_handle, cont, schema_handle, schema, ret);
	dsosd_rpc_complete(rpc, ret);
}
...
void rpc_handle_schema_add(dsosd_rpc_t *rpc)
{
	...
	cont_handle   = dsosd_rpc_unpack_handle(rpc);
	schema_handle = dsosd_rpc_unpack_handle(rpc);

	cont   = (sos_t)dsosd_handle_to_ptr(rpc, cont_handle);
	schema = (sos_schema_t)dsosd_handle_to_ptr(rpc, schema_handle);
	if (!cont || !schema)
		ret = EBADF;
	else
		ret = sos_schema_add(cont, schema);

	dsosd_debug("rpc %p: cont %lx/%p schema %p/%p\n",
		    rpc, cont_handle, cont, schema_handle, schema);
...
void rpc_handle_part_create(dsosd_rpc_t *rpc)
{
 ...
 out:
	dsosd_debug("rpc %p: cont %lx/%p name %s path %s ret %d\n",
		    rpc, handle, cont, name, path, ret);
	dsosd_rpc_complete(rpc, ret);
}
...
void rpc_handle_part_find(dsosd_rpc_t *rpc)
{
	...
	if (part)
		...
		ret = ENOENT;
 out:
	dsosd_debug("rpc %p: cont %lx/%p name %s part %p ret %d\n",
		    rpc, handle, cont, name, part, ret);
	dsosd_rpc_complete(rpc, ret);
}
...
void rpc_handle_part_set_state(dsosd_rpc_t *rpc)
{
	...
	part = (sos_part_t)dsosd_handle_to_ptr(rpc, handle);
	if (!part) {
		ret = EBADF;
		goto out;
	}
...
void rpc_handle_iter_close(dsosd_rpc_t *rpc)
{
	...
	iter = (sos_iter_t)dsosd_handle_to_ptr(rpc, handle);
	if (!iter) {
		ret = EBADF;
		goto out;
	}
...
void rpc_handle_iter_new(dsosd_rpc_t *rpc)
{
	...
	dsosd_debug("rpc %p: schema %lx/%p attr %p iter %p ret %d\n",
		    rpc, handle, schema, attr, iter, ret);
 out:
	dsosd_rpc_complete(rpc, ret);
}
...
void rpc_handle_iter_step(dsosd_rpc_t *rpc)
{
	...
	iter = dsosd_handle_to_ptr(rpc, handle);
	if (!iter) {
		ret = EBADF;
		goto out;
	}
	...
	dsosd_debug("rpc %p: op %d iter %lx/%p obj_va %lx obj_sz %ld key %p ret %d\n",
		    rpc, op, handle, iter, obj_va, obj_sz, key, ret);
 out:
	if (obj)
